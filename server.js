...
function addDepartment() {
  inquirer
    ...
    .then((answer) => {
      db.createNewDepartment(answer.name).then(() => {
...
function addEmployee() {
  ...
  db.findAllEmployees().then(({ rows }) => {
    ...
    const managerChoices = managers.map(({ id, first_name, last_name }) => ({
      // takes the managers array and maps through it to create the choices for prompt
    ...
    db.findAllRoles().then(({ rows }) => {
      // gets all employees
      const roles = rows; // stores full return of employees to the managers variable
      const roleChoices = roles.map(({ title, id }) => ({
        // takes the managers array and maps through it to create the choices for prompt

        name: title, //sets the employees first_name and last_name as what displays in the choice
        value: id, // sets the employees id as the value that gets sent when selected
      })); // ends the maping of a
      ...
      inquirer
        ...
        .then((answer) => {
          db.createNewEmployee(
            ...
            console.log(
              `Added employee ${answer.first_name} ${answer.first_last} to the database!`
            );
            // restart the application
            loadMainPrompts();
          });
